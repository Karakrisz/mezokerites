  ...
  <div class="bg-white rounded-lg flex shadow min-h-[250px] p-4 md:p-12 justify-center items-center">
    <div v-if="orders && orders.length" class="w-full">
      ...
      <div class="text-center flex justify-center w-full mt-8">
        <button type="button" class="flex items-center gap-1 text-sm leading-none hover:bg-stone-50 p-2 rounded" @click="refresh">
          <span>Reresh list</span>
          <Icon name="ion:refresh-outline" />
        </button>
      </div>
    </div>
    <div v-else-if="orders && orders.length === 0" class="min-h-[250px] flex items-center justify-center text-stone-500 text-lg">Nem található rendelés.</div>
