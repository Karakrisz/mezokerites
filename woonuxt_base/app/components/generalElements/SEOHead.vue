...
// Biztonságos SEO adatok lekérése
const seoArray = Array.isArray(wooNuxtSEO) ? wooNuxtSEO : [];
const facebook = seoArray.find((item) => item?.provider === 'facebook') ?? null;
const twitter = seoArray.find((item) => item?.provider === 'twitter') ?? null;

// Debug info (csak development módban)
if (isDev) {
  console.log('wooNuxtSEO:', wooNuxtSEO);
  console.log('wooNuxtSEO type:', typeof wooNuxtSEO);
  console.log('Is wooNuxtSEO array:', Array.isArray(wooNuxtSEO));
}
</script>
