...
<template>
  <div v-if="attributes" class="flex flex-col gap-1 justify-between">
    <div v-for="(attr, i) in attributes" :key="i" class="flex flex-wrap py-2 relative justify-between">
            ...
            <Tooltip :text="term.name">
              <label :for="`${term.slug}_${termIndex}`">
                <input
                  ...
                  :value="term.slug"
                  @change="updateAttrs" >
                <span class="color-button" :class="`color-${term.slug}`" :title="`${attr.name}: ${term}`"/>
              </label>
            </Tooltip>
          </span>
        </div>
      ...
      <div v-else-if="attr.terms.nodes && attr.terms.nodes?.length > 8" class="grid gap-2">
        ...
        <select :id="attr.name" :ref="attr.name" :name="attr.name" required class="border-white shadow" @change="updateAttrs">
          ...
          <option v-for="(term, dropdownIndex) in attr.terms.nodes" :key="dropdownIndex" :value="term.slug" :selected="dropdownIndex == 0" v-html="term.name" />
        </select>
      </div>
