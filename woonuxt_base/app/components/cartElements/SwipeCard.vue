<template>
  <div v-if="isAlive" class="rounded-lg flex h-24 w-full overflow-hidden relative items-center">
    <TrashIcon class="transform transition-all right-0 w-6 scale-0 absolute" :class="{ 'scale-100': lengthX > 40, 'delete-ready': lengthX > 80 }" />
    <div ref="el" class="rounded-lg inset-0 absolute" :class="{ 'transition-all': !isSwiping }" :style="{ transform: isSwiping ? `translateX(-${lengthX}px)` : `none` }">
      <slot />
    </div>
  </div>
</template>
