  ...
  <SwipeCard @remove="removeItem">
    <div v-if="productType" class="flex items-center gap-3 group">
      ...
      <div class="flex-1">
        ...
        <ProductPrice class=" text-xs" :sale-price="productType.salePrice" :regular-price="productType.regularPrice" />
      </div>
      <div class="inline-flex gap-2 flex-col items-end">
        <QuantityInput :item />
        <div class="text-xs text-stone-400 flex leading-none items-center">
          <button v-if="storeSettings.showMoveToWishlist" class="mr-2 pr-2 border-r hover:text-dark" type="button" @click="moveToWishList">Kívánságlistára</button>
          <button
            title="Remove Item"
